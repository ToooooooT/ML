...
def plot_eigenvector(image_path, U, a):
    fig = plt.figure()
    ...
    ax.set_xlabel('y')   
    ax.set_xlabel('z') 
    if image_path == './image1.png':
        plt.savefig('./eigenvector_normalized_kmeans++_image1.png')
    else:
        plt.savefig('./eigenvector_normalized_kmeans++_image2.png')
...
def normalized_spectral_clustering(image_path, gamma_s, gamma_c, k, init_type):
    ...
    Lsym = np.diag(1 / np.diag(np.sqrt(D))).dot(L).dot(np.diag(1 / np.diag(np.sqrt(D))))
    ...
    cluster_means = initalize(T, k, init_type)
    a, gifs = kmeans(cluster_means, k, T)

    # plot eigenvector
    if k == 3:
        plot_eigenvector(image_path, U, a)
...
if __name__ == '__main__':
    ...
    save_gif(gifs1, f'./GIF/{k}_normalized_spectral_clustering_{init_type}_image1.gif')
    save_gif(gifs2, f'./GIF/{k}_normalized_spectral_clustering_{init_type}_image2.gif')
