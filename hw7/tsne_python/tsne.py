...
def plot_iteration(y0, y1, perplexity):
    ...
    ani.save(f'tSNE_scatter_perplexity{perplexity}.gif', writer='pillow')


def plot_similarity(P, Q , perplexity):
    pylab.figure(figsize=(8, 10))
    pylab.subplot(2, 1 ,1)
    pylab.title('tSNE high-dimensional')
    pylab.hist(P.flatten(), bins=40, log=True)
    pylab.subplot(2, 1, 2)
    pylab.title('tSNE low-dimensional')
    pylab.hist(Q.flatten(), bins=40, log=True)
    pylab.savefig(f'tSNE_similarities_perplexity{perplexity}.png')
    pylab.clf()
...
if __name__ == "__main__":
    ...
    pylab.savefig(f'tSNE_scatter_perplexity{perplexity}.png')
